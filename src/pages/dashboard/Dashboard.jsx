...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data: clientsData, error: clientsError } = await supabase.from('clients').select('id, name').eq('agency_id', user.id)
        if (clientsError) throw clientsError
        
        const { data: reportsData, error: reportsError } = await supabase.from('reports').select('id, created_at, client_id')
